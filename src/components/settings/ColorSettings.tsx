...
export function ColorSettings() {
  ...
  return <div className="px-4 py-[13px]">
      ...
      <div className="flex flex-col gap-2">
        ...
        <Button variant="outline" className={`justify-start h-9 ${colorMode === 'pink' ? 'bg-accent' : ''}`} onClick={() => handleColorChange('pink')}>
          <div className="w-4 h-4 rounded-full bg-[hsl(328,73%,69%)] mr-2" />
          Pink
        </Button>
        <Button variant="outline" className={`justify-start h-9 ${colorMode === 'purple' ? 'bg-accent' : ''}`} onClick={() => handleColorChange('purple')}>
