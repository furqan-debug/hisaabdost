...
export function TrendsCard({
  ...
  const insights = useMemo(() => {
    ...
    const currentPeriodExpenses = expenses.filter(expense => {
      const expenseDate = parseISO(expense.date);
      const currentPeriodStart = viewType === 'weekly' ? startOfWeek(new Date()) : viewType === 'quarterly' ? startOfQuarter(new Date()) : startOfMonth(new Date());
    ...
    const topCategory = Object.entries(categoryTotals).sort(([, a], [, b]) => Number(b) - Number(a))[0];
      ...
      <Card className="h-full border-0 shadow-lg bg-gradient-to-br from-card/95 to-card/80 backdrop-blur-md">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            {insights && <div className={`flex items-center gap-1 text-sm ${insights.trend === 'up' ? 'text-red-500' : insights.trend === 'down' ? 'text-green-500' : 'text-muted-foreground'}`}>
                {insights.trend === 'up' && <TrendingUp className="h-3 w-3" />}
                {insights.trend === 'down' && <TrendingDown className="h-3 w-3" />}
                {insights.percentChange !== 0 && <span className="font-medium">
          ...
          <div className="flex gap-1 mt-2">
            {(['weekly', 'monthly', 'quarterly'] as ViewType[]).map(type => <Button key={type} variant={viewType === type ? "default" : "ghost"} size="sm" onClick={() => setViewType(type)} className="h-7 px-2 text-xs">
            ...
            y: 0
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      {insights.topCategory && <div className="flex items-center gap-2 mt-2">
              ...
              <div className="relative">
                <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent" style={{
              minWidth: '100%',
              maxWidth: '100%'
            }}>
                  <div style={{
                ...
                height: '320px'
                    ...
                    bottom: 80
                  }} barCategoryGap={viewType === 'weekly' ? "35%" : "25%"} maxBarSize={isMobile ? 35 : 50}>
                        <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.2} className="stroke-muted" />
                        <XAxis dataKey="label" axisLine={false} tickLine={false} tick={{
                      fontSize: isMobile ? 10 : 11,
                      fill: 'hsl(var(--muted-foreground))',
                      textAnchor: 'end'
                    }} interval={0} angle={-35} height={80} className="text-muted-foreground" />
                      ...
                      return <motion.div initial={{
                        ...
                        y: 0
                                ...
                                <div className="text-center space-y-2">
                                  ...
                                  {data.amount > 0 && <div className="text-xs text-muted-foreground">
                                      {data.amount > 1000 ? 'High spending' : data.amount > 500 ? 'Moderate spending' : 'Low spending'}
                                    </div>}
                                </div>
                              </motion.div>;
                    }} cursor={{
                      ...
                      opacity: 0.1
                        ...
                        <Bar dataKey="amount" radius={[6, 6, 0, 0]} label={({
                      value,
                      x,
                      y,
                      width
                      ...
                      return <text x={x + width / 2} y={y - 8} fill="hsl(var(--muted-foreground))" textAnchor="middle" fontSize="10" fontWeight="600" className="drop-shadow-sm">
