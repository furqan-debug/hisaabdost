...
export function ExpensesBarChart({
  ...
  return <div className="w-full h-[250px] md:h-[300px]">
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={chartData} className="mx--16 my--4 ">
          ...
          return <div className="rounded-lg border bg-background p-2 shadow-sm">
                  ...
                  {payload.map(entry => <p key={entry.name} className="text-sm" style={{
              color: entry.color
            }}>
          ...
          <Legend />
          {Object.keys(CATEGORY_COLORS).map(category => <Bar key={category} dataKey={category} stackId="a" fill={CATEGORY_COLORS[category as keyof typeof CATEGORY_COLORS]} />)}
        </BarChart>
      </ResponsiveContainer>
    </div>;
}