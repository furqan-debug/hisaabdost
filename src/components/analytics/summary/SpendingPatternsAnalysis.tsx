...
export function SpendingPatternsAnalysis({
  spendingPatterns,
  totalSpending
}: SpendingPatternsAnalysisProps) {
  const {
    currencyCode
  } = useCurrency();
  ...
  return <Card>
      ...
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {spendingPatterns.map((pattern, index) => <motion.div key={pattern.pattern} initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          delay: index * 0.1
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium px-[6px]">{formatAmount(pattern.averageAmount)}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                {pattern.totalAmount * 12 > 5000 && <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded">
