...
export const BudgetTabs = ({
  ...
  return <motion.div className="w-full space-y-6" initial={{
    opacity: 0,
    y: 10
  }} animate={{
    opacity: 1,
    ...
    duration: 0.3
      ...
      <Tabs value={stableActiveTab} onValueChange={handleValueChange} className="w-full">
        <div className="w-full flex justify-center mb-6">
          <TabsList className="bg-muted/50 backdrop-blur-sm border border-border/40 p-1 rounded-xl grid w-full max-w-md grid-cols-4 gap-1 my-[7px] px-0 py-0">
            {tabs.map(tab => <TabsTrigger key={tab.id} value={tab.id} className="flex flex-col items-center justify-center gap-1 rounded-lg px-2 text-xs font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm hover:bg-background/50 py-[3px]">
