...
export function BudgetOverview({
  ...
  return <motion.div initial={{
    ...
    y: 0
      ...
      <Card className="border-0 shadow-lg bg-gradient-to-br from-card/95 to-card/85 backdrop-blur-sm overflow-hidden px-0">
            ...
            <div className="relative">
              <div className="aspect-square max-w-[350px] mx-auto relative">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={data} cx="50%" cy="50%" innerRadius={isMobile ? 60 : 80} outerRadius={isMobile ? 90 : 120} paddingAngle={3} dataKey="value" cornerRadius={6} strokeWidth={0}>
                      {data.map((entry, index) => <Cell key={`cell-${index}`} fill={CATEGORY_COLORS[entry.name]} />)}
                    </Pie>
                    <Tooltip content={({
                    active,
                    payload
                    ...
                    return <div className="bg-background/95 backdrop-blur-sm border border-border/50 rounded-xl p-4 shadow-xl">
