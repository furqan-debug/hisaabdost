...
export const ExpenseBarChart = ({ expenses }: ExpenseBarChartProps) => {
  ...
  // This logic now correctly gets all active categories, not just a slice
  const allActiveCategories = activeCategories;
  
  // The outer div has been removed, as the parent ChartContainer now handles sizing.
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        />
        <YAxis 
          // THIS IS THE CRITICAL FIX FOR "LONG BARS"
          domain={[0, 'dataMax']}
          allowDataOverflow={false} // Prevents drawing outside the area
          axisLine={false}
          tickLine={false}
          tick={{ fontSize: isMobile ? 10 : 12, fill: 'var(--foreground)' }}
