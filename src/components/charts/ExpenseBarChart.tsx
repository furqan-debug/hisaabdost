...
export const ExpenseBarChart = ({ expenses }: ExpenseBarChartProps) => {
  ...
  // FIX #1: Removed the ".slice(0, 3)" to show ALL active categories, not just the top 3.
  const allActiveCategories = activeCategories;
  
  // FIX #2: Removed the outer div with a fixed height.
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        />
        <YAxis 
          // FIX #3: Added domain to prevent bars from drawing below the 0 line.
          domain={[0, 'dataMax']}
          axisLine={false}
          tickLine={false}
          tick={{ fontSize: isMobile ? 10 : 12, fill: 'var(--foreground)' }}
