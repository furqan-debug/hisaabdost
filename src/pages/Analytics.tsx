...
export default function Analytics() {
  ...
  const generateInsights = () => {
    ...
    if (highestCategory) {
      insights.push({
        type: 'category',
        message: `Your highest spending category is ${highestCategory[0]} at ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(highestCategory[1])}`
      });
    }

    // Find transactions above average
    const highTransactions = filteredExpenses.filter(exp => Number(exp.amount) > avgExpense);
    if (highTransactions.length > 0) {
      insights.push({
        type: 'transaction',
        message: `You have ${highTransactions.length} transactions above your average expense of ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(avgExpense)}`
      });
    }
