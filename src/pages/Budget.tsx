...
const Budget = () => {
  ...
  if (isLoading) {
    return <div className="p-4 flex justify-center min-h-[200px]">
        <div className="animate-pulse text-center">
          <p className="text-muted-foreground">Loading your budget data...</p>
        </div>
      </div>;
  }
  return <div className="overflow-guard space-y-3 pb-20 md:pb-8 min-h-[100dvh] w-full overflow-x-hidden budget-container">
      <div className="sticky top-0 bg-background/95 backdrop-blur-sm z-10 px-3 py-2">
        <BudgetHeader onAddBudget={handleAddBudget} onExport={exportBudgetData} />
      </div>

      <div className="px-3">
        <BudgetSummaryCards totalBudget={totalBudget} remainingBalance={remainingBalance} usagePercentage={usagePercentage} monthlyIncome={monthlyIncome} isLoading={isLoading} />
      </div>

      <div className="w-full min-h-[350px] budget-content my-[14px]">
        <BudgetTabs budgets={budgets || []} onEditBudget={handleEditBudget} activeTab={activeTab} onTabChange={handleTabChange} />
      </div>

      <BudgetForm open={showBudgetForm} onOpenChange={setShowBudgetForm} budget={selectedBudget} onSuccess={() => {
    ...
    }} monthlyIncome={monthlyIncome} totalBudget={totalBudget} />
    </div>;
};
